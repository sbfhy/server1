...
protected:
    ...
    enum ToolMode                           // Test Navmesh的类型
    {
        ...
        TOOLMODE_PATHFIND_STRAIGHT,         // 查找从起点到终点的直线路径
    ...
    int m_straightPathOptions;              // dtStraightPathOptions, 用于dtNavMeshQuery::findStraightPath的参数

    static const int MAX_POLYS = 256;       // 多边形最大个数
    static const int MAX_SMOOTH = 2048;     // 平滑路径最大数量

    dtPolyRef m_startRef;                   // 起点多边形
    dtPolyRef m_endRef;                     // 终点多边形
    dtPolyRef m_polys[MAX_POLYS];           // 所有多边形
    ...
    float m_distanceToWall;                 // 到墙壁距离 

    float m_neighbourhoodRadius;            // 半径，TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD 起点周围的多边形
    // float m_randomRadius;
    bool m_sposSet;                         // 是否设置了起点
    bool m_eposSet;                         // 是否设置了终点
